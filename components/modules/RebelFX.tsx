...
export const RebelFXModule: React.FC<RebelFXProps> = ({ initialState, onStateChange }) => {
  ...
  // Signal State
  const [signalSettings, setSignalSettings] = useState<SignalSettings>({ pair: 'XAUUSD', timeframe: 'H1' });
  const [generatedSignal, setGeneratedSignal] = useState<any>(null);
  const [isSignaling, setIsSignaling] = useState(false);

  // Signal Settings History
  const [signalHistory, setSignalHistory] = useState<SignalSettings[]>([]);
  const [signalHistoryIndex, setSignalHistoryIndex] = useState(-1);
  ...
  const updateSignalSettings = (newSettings: Partial<SignalSettings>) => {
    const updated = { ...signalSettings, ...newSettings };
    setSignalSettings(updated);
    
    if (JSON.stringify(updated) !== JSON.stringify(signalHistory[signalHistoryIndex])) {
        const newHistory = [...signalHistory.slice(0, signalHistoryIndex + 1), updated];
        setSignalHistory(newHistory);
        setSignalHistoryIndex(newHistory.length - 1);
    }
  };

  const handleUndoSignal = () => {
    if (signalHistoryIndex > 0) {
      const newIndex = signalHistoryIndex - 1;
      setSignalSettings(signalHistory[newIndex]);
      setSignalHistoryIndex(newIndex);
    }
  };

  const handleRedoSignal = () => {
    if (signalHistoryIndex < signalHistory.length - 1) {
      const newIndex = signalHistoryIndex + 1;
      setSignalSettings(signalHistory[newIndex]);
      setSignalHistoryIndex(newIndex);
    }
  ...
  useEffect(() => {
    if (initialState) {
        setMessages(initialState.messages || messages);
        setScanResult(initialState.scanResult || '');
        setGeneratedSignal(initialState.generatedSignal || null);
        if (initialState.signalHistory && initialState.signalHistory.length > 0) {
            setSignalHistory(initialState.signalHistory);
            const newIndex = initialState.signalHistoryIndex ?? initialState.signalHistory.length - 1;
            setSignalHistoryIndex(newIndex);
            setSignalSettings(initialState.signalHistory[newIndex]);
        }
    } else {
        setSignalHistory([signalSettings]);
        setSignalHistoryIndex(0);
    }
  }, [initialState]);
  ...
  }, [messages, scanResult, generatedSignal, signalHistory, signalHistoryIndex]);
  ...
  const handleGenerateSignal = async () => {
    ...
    try {
      const jsonStr = await generateForexSignal(signalSettings.pair, signalSettings.timeframe);
      ...
      <header className="bg-slate-900/80 backdrop-blur-md border-b border-slate-800 p-4 sticky top-0 z-50">
        <div className="flex justify-between items-center max-w-7xl mx-auto">
          <div className="flex items-center gap-3">
             ...
             <div>
               <h1 className="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500 tracking-wider">Smart PandaFX</h1>
               <p className="text-[10px] text-slate-500 font-mono tracking-widest uppercase">Asisten trading pribadimu! Analisa pasar, ngobrol bareng AI Astra, dan dapatkan sinyal potensial.</p>
             </div>
          </div>
                 ...
                 <div className="h-full max-w-3xl mx-auto animate-fade-in space-y-6">
                    <div className="bg-slate-900 rounded-3xl p-6 border border-slate-800 shadow-xl">
                       ...
                       <div className="flex gap-4 mb-6">
                          <select 
                             value={signalSettings.pair}
                             onChange={(e) => updateSignalSettings({ pair: e.target.value })}
                          ...
                          <select 
                             value={signalSettings.timeframe}
                             onChange={(e) => updateSignalSettings({ timeframe: e.target.value })}
                             className="flex-1 bg-slate-800 border border-slate-700 text-white rounded-xl p-3 outline-none focus:border-cyan-500"
                          >
                             {TIMEFRAMES.map(t => <option key={t} value={t}>{t}</option>)}
                          </select>
                          <div className="flex items-center gap-2">
                            <button onClick={handleUndoSignal} disabled={signalHistoryIndex <= 0} className="p-3 bg-slate-800 text-slate-400 rounded-xl disabled:opacity-30 hover:bg-slate-700"><RotateCcw size={18}/></button>
                            <button onClick={handleRedoSignal} disabled={signalHistoryIndex >= signalHistory.length - 1} className="p-3 bg-slate-800 text-slate-400 rounded-xl disabled:opacity-30 hover:bg-slate-700"><RotateCw size={18}/></button>
                            <button 
                                onClick={handleGenerateSignal}
                                disabled={isSignaling}
                                className="px-4 py-3 bg-cyan-600 hover:bg-cyan-500 text-white font-bold rounded-xl transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
                            >
