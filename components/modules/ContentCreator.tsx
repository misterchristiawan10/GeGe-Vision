...
import { Layers, Sparkles, Loader2, Wand2, ZoomIn, Download, X, History } from 'lucide-react';
...
interface FlatLayHistory {
    id: number;
    image: string;
}

export const ContentCreatorModule: React.FC<ContentCreatorProps> = ({ onNavigate, onTransfer, initialState, onStateChange }) => {
  ...
  // History & Lightbox
  const [flatLayHistory, setFlatLayHistory] = useState<FlatLayHistory[]>([]);
  ...
  useEffect(() => {
    if (initialState) {
      ...
      setFlatLayResult(initialState.flatLayResult || null);
      setFlatLayHistory(initialState.flatLayHistory || []);
      setSocialPlatform(initialState.socialPlatform || 'Instagram');
      setCaptionResult(initialState.captionResult || '');
    }
  }, [initialState]);
  ...
  }, [analysisResult, flatLayResult, flatLayHistory, socialPlatform, captionResult]);
  ...
  const handleFlatLay = async () => {
    ...
    try {
      ...
      // Add to history
      setFlatLayHistory(prev => [{ id: Date.now(), image: result }, ...prev]);
  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-5xl mx-auto pb-10">
        ...
        >
           ...
           <div className="relative max-w-7xl max-h-[90vh] w-full h-full flex flex-col items-center justify-center" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="mt-4 flex gap-4">
                 <a href={viewImage} download={`flatlay-${Date.now()}.png`} className="flex items-center gap-2 px-6 py-3 bg-white text-gray-900 rounded-full font-bold shadow-lg hover:bg-gray-200 transition-colors">
      ...
      <div className="text-center space-y-2">
        <h2 className="text-3xl font-bold text-gray-900 dark:text-white">Smart Analisa</h2>
        <p className="text-gray-500 dark:text-gray-400">Bingung mau bikin konten apa? Upload gambarmu, biar AI kasih ide prompt & caption medsos yang nendang.</p>
      </div>
        ...
        <div className="bg-white dark:bg-dark-card p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 h-full flex flex-col">
          ...
          <div className="flex-1 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl p-4 flex flex-col justify-center items-center relative bg-gray-50 dark:bg-gray-800/50 min-h-[300px]">
             <input 
                ...
                className="hidden" 
                id="content-creator-upload" 
             />
             <label htmlFor="content-creator-upload" className="cursor-pointer w-full h-full flex flex-col items-center justify-center">
                {previewUrl ? (
      ...
      <div className="bg-white dark:bg-dark-card p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
         ...
         <div className="flex flex-col md:flex-row gap-6">
            <div className="flex-1 flex flex-col gap-4">
                ...
                className={`w-full py-4 rounded-xl font-bold text-white shadow-lg transition-all transform duration-300 flex items-center justify-center gap-2 overflow-hidden group
                    ${!image || isFlatLaying 
                    ? 'bg-gray-300 dark:bg-gray-700 cursor-not-allowed opacity-80' 
                    : 'bg-gradient-to-br from-emerald-500 via-teal-500 to-cyan-600 hover:from-emerald-600 hover:via-teal-600 hover:to-cyan-700'
                    }
                `}
                >
                {isFlatLaying ? (
                ...
                {flatLayHistory.length > 0 && (
                    <div className="border-t border-gray-100 dark:border-gray-700 pt-4">
                        ...
                        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                            {flatLayHistory.map((item) => (
                                <div key={item.id} className="relative w-16 h-16 rounded-lg overflow-hidden flex-shrink-0 cursor-pointer border border-gray-200 dark:border-gray-700 group" onClick={() => setViewImage(item.image)}>
                                    <img src={item.image} className="w-full h-full object-cover" alt="Hist" />
                                </div>
                            ))}
                        </div>
                    </div>
            ...
            <div className="flex-1 bg-gray-50 dark:bg-gray-800 rounded-xl min-h-[150px] flex flex-col gap-4 items-center justify-center border border-gray-200 dark:border-gray-700 p-4 relative group cursor-pointer" onClick={() => flatLayResult && setViewImage(flatLayResult)}>
               {flatLayResult ? (
                  <>
                     <div className="w-full flex-1 flex items-center justify-center relative">
                        <img src={flatLayResult} alt="Flat Lay" className="max-h-[300px] w-auto object-contain rounded-lg" />
                        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 flex items-center justify-center transition-colors opacity-0 group-hover:opacity-100">
                     ...
                     <div className="flex gap-2 self-center" onClick={(e) => e.stopPropagation()}>
                       ...
                       >
                          <span>✨</span> Transfer ke Smart Foto Studio
                       </button>
                       <a 
                         href={flatLayResult} 
