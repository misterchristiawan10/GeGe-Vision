...
import { 
  ...
  Settings as SettingsIcon, Key, Cpu, Sun, Moon, ExternalLink
} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-gray-50 dark:bg-dark-bg transition-colors duration-300 overflow-x-hidden">
        ...
        <div className="h-full flex flex-col">
          ...
          <nav className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar">
            ...
            >
             ...
             {/* Key Management Button - Unified into Settings */}
            <button 
                onClick={() => handleNavClick('settings')}
                className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl transition-all group ${activeModuleId === 'settings' ? 'bg-violet-600 text-white shadow-lg' : 'bg-violet-600/5 hover:bg-violet-600/10 border border-violet-500/20 text-violet-600 dark:text-violet-400'}`}
            >
                <div className={`p-1.5 rounded-lg shadow-lg group-hover:scale-110 transition-transform ${activeModuleId === 'settings' ? 'bg-white text-violet-600' : 'bg-violet-600 text-white'}`}>
                    <SettingsIcon size={14} />
                </div>
                <div className="text-left">
                    <p className={`text-[10px] font-black uppercase tracking-widest leading-none mb-1 ${activeModuleId === 'settings' ? 'text-white' : ''}`}>Settings & Vault</p>
                    <p className={`text-[9px] opacity-70 ${activeModuleId === 'settings' ? 'text-violet-100' : ''}`}>Kelola Akun & API</p>
                </div>
            </button>
          ...
          <div className="p-4 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-100 dark:border-gray-800">
            {user && (
              <div className="flex items-center gap-3 mb-4 p-2.5 rounded-2xl bg-white dark:bg-gray-800 shadow-sm border border-gray-100 dark:border-gray-700 cursor-pointer hover:border-violet-500 transition-colors" onClick={() => handleNavClick('settings')}>
      ...
      <main className="flex-1 flex flex-col min-w-0 min-h-screen overflow-x-hidden">
        {/* Mobile Header */}
        <header className="sticky top-0 z-40 w-full bg-white/80 dark:bg-dark-card/80 backdrop-blur-md border-b border-gray-100 dark:border-gray-800 lg:hidden">
          <div className="flex items-center justify-between p-4">
            <div className="flex items-center gap-2" onClick={() => handleNavClick('home')}>
              <GeGePrismLogo size="sm" />
              <h1 className="text-sm font-black tracking-tighter text-gray-900 dark:text-white">GEGE VISION</h1>
            </div>
            <button onClick={() => setIsMobileMenuOpen(true)} className="p-2 text-gray-400 hover:text-violet-500 transition-colors">
              <Menu size={20}/>
            </button>
          </div>
        </header>
